...
from worker import WORKER_METRICS, process_log, run_worker
...
@patch("worker.get_vector_store")
@patch("worker.get_embedding_model")
def test_process_log_valid_json(mock_get_model, mock_get_vector_store, caplog):
    reset_metrics()
    ...
    assert "Processing log | level=ERROR | parser=structured_json" in caplog.text
    assert "Successfully vectorized and indexed log to Qdrant" in caplog.text

    mock_store.ensure_collection.assert_called_once_with(vector_size=384)
    mock_store.upsert_log.assert_called_once()

    payload = mock_store.upsert_log.call_args.kwargs["payload"]
    vector = mock_store.upsert_log.call_args.kwargs["vector"]

    assert payload["service_id"] == "auth-service"
    assert payload["level"] == "ERROR"
    assert payload["message"] == "Out of memory exception at address 0x1234"
...
@patch("worker.get_vector_store")
@patch("worker.get_embedding_model")
def test_process_log_embedding_failure(mock_get_model, mock_get_vector_store, caplog):
    reset_metrics()
    caplog.set_level(logging.ERROR)

    mock_model = MagicMock()
    mock_model.encode.side_effect = Exception("Model loading error")
    mock_get_model.return_value = mock_model
...
@patch("worker.get_vector_store")
@patch("worker.get_embedding_model")
def test_process_log_qdrant_failure(mock_get_model, mock_get_vector_store, caplog):
    reset_metrics()
    caplog.set_level(logging.ERROR)
    ...
    mock_store = MagicMock()
    mock_store.upsert_log.side_effect = Exception("Qdrant write timeout")
    mock_get_vector_store.return_value = mock_store
...
@patch("worker.get_vector_store")
@patch("worker.get_embedding_model")
def test_process_log_missing_service_uses_unknown_service(
    mock_get_model,
    ...
    stored_payload = mock_store.upsert_log.call_args.kwargs["payload"]
    assert stored_payload["service_id"] == "unknown_service"
